...
################################################################################
# run_protoc_java.sh protoc编译脚本(java专用)
#
# 作者: zhaochun
# 日期: 2022/12/13
# 描述: 本脚本用于编译将proto工程编译为java工程，使用时注意:
...
# 指定go代码工程目录(相对当前目录)
read -r -p "请输入java代码工程目录(相对当前目录), 默认 ${default_java_out}:" java_out
if [ "$proto_path" == "" ]
then
...
if [ "$java_out" == "" ]
then
  java_out=${default_java_out}
fi
echo
echo "--- 编译目录情报汇总 开始 ---"
echo "您现在位于: $(pwd)"
echo "您指定的proto工程目录(相对当前目录): "$proto_path
echo "您指定的java代码工程目录(相对当前目录): "$java_out
echo "--- 编译目录情报汇总 结束 ---"
echo
...
echo
echo "===== 1. 删除 java_out 下的即存编译结果 ====="

# 删除目标工程下的代码文件
# 用find命令寻找`${java_out}`下的目录。
#  `-maxdepth 1 -mindepth 1`表示只向下寻找一层。
#  `-type d`表示只寻找目录。
#  `\( ! -iname ".*" \)`表示排除掉隐藏目录或文件。
# find结果借由管道传递给`while ... do`，对其结果一行一行处理。
#  `read -r line`表示将当前行写入`line`变量，`-r`表示禁止反斜杠转义。
find ${java_out} -maxdepth 1 -mindepth 1 -type d \( ! -iname ".*" \) | while read -r line; do
    echo "删除: $line"
    rm -rf "$line"
done
